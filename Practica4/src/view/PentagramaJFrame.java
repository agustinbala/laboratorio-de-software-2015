...
public class PentagramaJFrame {
	
	private Figura figura;
	private Nota nota;
	private JTextField labelNotas;
	...
	private void setMargin(JPanel component, Integer margin){
	...
	private void setSelectedNote(MouseEvent arg0) {
		if (arg0.getY() <= 310
				&& arg0.getY() >= 280) {
			nota = Nota.C;
		} else {
			if (arg0.getY() <= 280
				...
				if (arg0.getY() <= 250 && arg0.getY() >= 220) {
					nota = Nota.E;
				} else {
					if (arg0.getY() <= 220 && arg0.getY() >= 190) {
						nota = Nota.F;
					} else {
						if ( arg0.getY() <= 190 && arg0.getY() >= 160) {
							nota = Nota.G;
						} else {
							if (arg0.getY() <= 160 && arg0.getY() >= 130) {
								nota = Nota.A;
							} else {
								if ( arg0.getY() <= 130
										&& arg0.getY() >= 100) {
	...
	private JPanel createPanel(final Figura figuraTemp)
			throws IOException {
        ...
        label.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				PentagramaJFrame.this.figura = figuraTemp;
			}
		});
		panel.add(label);
